...
mod aruna_connector_negotiation_provider_api_test {
    use crate::common::{setup_dsp_provider_configuration, setup_management_consumer, setup_provider_configuration, setup_random_contract_negotiation, DATASPACE_PROTOCOL, PROVIDER_ID, PROVIDER_PROTOCOL};
    ...
    async fn test_get_negotiation() {
        ...
        let (negotiation, _) = setup_random_contract_negotiation(&consumer, &provider).await;
        let response = contract_negotiation_api::get_negotiation(&consumer, negotiation.as_str()).await;
