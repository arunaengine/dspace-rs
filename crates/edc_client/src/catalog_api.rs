...
pub async fn get_dataset(
    ...
    dataset_request: Option<edc_api::DatasetRequest>,
) -> Result<serde_json::Value, Error<GetDatasetError>> {
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<GetDatasetError> =
...
pub async fn request_catalog(
    configuration: &configuration::Configuration,
    catalog_request: Option<edc_api::CatalogRequest>,
) -> Result<serde_json::Value, Error<RequestCatalogError>> {
    let local_var_configuration = configuration;
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<RequestCatalogError> =
            serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent {
            status: local_var_status,
            content: local_var_content,
