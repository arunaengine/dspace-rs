...
pub async fn request_transfer_processes(headers: HeaderMap, State(state): State<SharedState>, Json(request_message): Json<Value>,) -> impl IntoResponse {
    ...
    debug!("[DSP] Initialized new Transfer Process State Machine; State: {}", fsm.state);
    ...
    debug!("[DSP] Contract Negotiation Provider PID: {:#?}", provider_pid.clone());
    println!("[DSP] Transfer Process FSM: {:#?}", fsm.clone()); // only for demonstration purposes, remove later

    state.insert(provider_pid.clone(), (transfer_process.clone(), fsm.clone()));
