...
fn evaluate_condition(contract: &ContractAgreement, operand_left: &serde_json::Value, operator: &str, operand_right: &serde_json::Value,) -> bool {
    ...
    match field_name {
        ...
        "contractSigningDate" => compare_values(contract.contract_signing_date, operator, operand_right.as_i64()),
        "consumerId" => compare_values(contract.consumer_id.as_deref(), operator, operand_right.as_str()),
        "providerId" => compare_values(contract.provider_id.as_deref(), operator, operand_right.as_str()),
...
pub(crate) async fn request_agreements(State(state): State<SharedState>, Json(query): Json<QuerySpec>,) -> impl IntoResponse {
    ...
    debug!("Received Contract Agreement request for query: {:#?}", query);
...
pub(crate) async fn get_agreement(State(state): State<SharedState>, Path(id): Path<String>) -> impl IntoResponse {
    ...
    debug!("Received Contract Agreement request for id: {:#?}", id.clone());

    let state = state.lock().await;
    match state.get(&id) {
        Some(output) => (StatusCode::OK, Json(output.clone())).into_response(),
        None => (StatusCode::NOT_FOUND, Json(error!("A contract agreement with the given ID does not exist"))).into_response(),
    }

}

pub(crate) async fn get_agreement_negotiation(State(state): State<SharedState>, Path(id): Path<String>) -> impl IntoResponse {
    ...
    debug!("Received Contract Negotiation by Agreement request for id: {:#?}", id.clone());

    let state = state.lock().await;

    if state.contains_key(&id) {

        let url = format!("http://localhost:3000/v2/contractnegotiations/{}", id.clone());
        ...
        match response {
            ...
            Err(e) => {
                error!("Error: {:#?}", e);
                (StatusCode::INTERNAL_SERVER_ERROR, Json(error!("An error occurred while fetching the contract negotiation"))).into_response()
            }
        }
    } else {
        (StatusCode::NOT_FOUND, Json(error!("A contract agreement with the given ID does not exist"))).into_response()
    }
