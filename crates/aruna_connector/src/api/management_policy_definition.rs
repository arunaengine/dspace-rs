...
use edc_api::{
    ContractDefinitionOutput, Criterion, IdResponse, PolicyDefinitionInput, PolicyDefinitionOutput,
    QuerySpec,
};
use reqwest::Client;
use serde_json::json;
...
pub(crate) async fn create_policy_definition(
    State(state): State<SharedState>,
    Json(input): Json<PolicyDefinitionInput>,
) -> impl IntoResponse {
...
pub(crate) async fn request_policy_definitions(
    State(state): State<SharedState>,
...
pub(crate) async fn get_policy_definition(
    State(state): State<SharedState>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    ...
    debug!(
        "Received Policy Definition request for id: {:#?}",
        id.clone()
    );
...
pub(crate) async fn update_policy_definition(
    State(state): State<SharedState>,
    Json(input): Json<PolicyDefinitionInput>,
) -> impl IntoResponse {
...
pub(crate) async fn delete_policy_definition(
    State(state): State<SharedState>,
    Path(id): Path<String>,
) -> impl IntoResponse {
    ...
    debug!(
        "Received Policy Definition deletion request for Policy Definition with id: {:#?}",
        id.clone()
    );
    ...
    match response {
        ...
        _ => {
            return (
                ...
                Json(error!(
                    "Failed to verify Contract Definition references for the Policy Definition."
                )),
            )
                .into_response();
        }
