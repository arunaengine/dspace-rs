...
use dsp_api::contract_negotiation::{AbstractPolicyRule, Action, ContractNegotiation, ContractNegotiationTerminationMessage, ContractOfferMessage, MessageOffer, Permission, PolicyClass, Target};
...
pub async fn get_negotiation(State(state): State<SharedState>, Path(pid): Path<String>) -> impl IntoResponse {
    ...
    debug!("[DSP] Received Negotiation request for pid {:#?}", pid.clone());
...
pub async fn request_negotiation(headers: HeaderMap, State(state): State<SharedState>, Json(request_message): Json<Value>) -> impl IntoResponse {
    ...
    if let Some(provider_pid) = request_message["provider_pid"].as_str() {
        ...
        debug!("[DSP] Contract Negotiation Provider PID: {:#?}", pid.clone());
...
pub async fn terminate_negotiation(headers: HeaderMap, State(state): State<SharedState>, Path(pid): Path<String>, Json(termination_request): Json<Value>,) -> impl IntoResponse {
    info!("[DSP] Terminate Negotiation called");
    debug!("[DSP] Received Negotiation termination request for pid {:#?}", pid.clone());
    ...
    match state.get(&pid) {
        Some((negotiation, _)) => {
            ...
            debug!("[DSP] Received Contract Negotiation termination for id {:#?} with reason {}", pid.clone(), reason.clone());
            ...
            let negotiation = ContractNegotiation {
                context: default_context,
                dsp_type: "dspace:ContractNegotiation".to_string(),
                consumer_pid: termination_request["dspace:consumerPid"].to_string().clone(),
                provider_pid: pid.clone(),
            ...
            let transition_message = format!("Requesting Contract Negotiation Termination from {}", host.clone());
            fsm.transition_to_terminating(transition_message.as_str());
            let transition_message = format!("Terminated after Contract Negotiation Termination request from {} with reason {}", host.clone(), reason.clone());
