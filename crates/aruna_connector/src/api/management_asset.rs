...
pub(crate) async fn update_asset(
    ...
    if state.contains_key(&id) {
        ...
        (
            ...
            Json(error!(
                "Asset could not be updated, because an Asset with the given id does not exist."
            )),
        )
            .into_response()
    }
...
pub(crate) async fn create_asset(
    ...
    if state.contains_key(&id) {
        return (
            StatusCode::CONFLICT,
            Json(error!(
                "Could not create asset, because an asset with the given ID already exists"
            )),
        )
            .into_response();
    }
...
pub(crate) async fn request_assets(
    State(state): State<SharedState>,
...
pub(crate) async fn get_asset(
    ...
    match state.get(&id) {
        ...
        None => (
            StatusCode::NOT_FOUND,
            Json(error!("An Asset with the given ID does not exist")),
        )
            .into_response(),
    }
}
...
pub(crate) async fn delete_asset(
    ...
    debug!(
        "Received Asset deletion request for Asset with id: {:#?}",
        id.clone()
    );
    ...
    if state.remove(&id).is_some() {
        ...
        (
            StatusCode::NOT_FOUND,
            Json(error!(
                "Asset could not be deleted, because an Asset with the given id does not exist."
            )),
        )
            .into_response()
    }
